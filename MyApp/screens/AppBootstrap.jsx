...
export default function AppBootstrap({ navigation }) {
  ...
  const navigateWhenReady = useCallback(() => {
    ...
    navigation.replace(pendingRouteRef.current);
  }, [navigation]);
  ...
  useEffect(() => {
    ...
    const brandAnimation = Animated.sequence([
      Animated.delay(2000),
  ...
  }, [brandProgress, fade, navigateWhenReady, pulse, scale]);

  useEffect(() => {
    let active = true;

    // Start waking the deployed API while the splash animation is playing.
    // This is intentionally non-blocking; local startup determines the route.
    api.get("/health", { timeout: 60000 }).catch((err) => {
      if (__DEV__) console.log("[startup] API warm-up:", err?.message);
    });
    ...
    const boot = async () => {
      ...
      const getStartedSeen = stored.hasSeenGetStarted || stored.getStartedSeen;
      const privacyAccepted =
        stored.hasAcceptedPrivacy ||
        stored.hasAcceptedDataPrivacy ||
        stored.privacyAccepted;
      const termsAccepted = stored.hasAcceptedTerms || stored.termsAccepted;
      const hasCreatedAccount = stored.hasAccount || stored.hasCreatedAccount;
      const onboardingComplete = stored.onboardingComplete;
    ...
    boot().catch((err) => {
      if (__DEV__) console.log("[startup] local initialization failed:", err?.message);
      if (active) {
  ...
  }, [navigateWhenReady, user?._id, user?.id]);
