...
const WEATHER_CACHE_KEY = "@sagipbayan/jaen-weather-v1";
let weatherMemoryCache = null;
...
async function readCachedWeather() {
  if (weatherMemoryCache) return weatherMemoryCache;

  try {
    const stored = await AsyncStorage.getItem(WEATHER_CACHE_KEY);
    const cached = stored ? JSON.parse(stored) : null;
    if (!cached?.weather?.current) return null;
    weatherMemoryCache = cached.weather;
...
function requestFreshWeather(weatherUrl) {
  if (weatherRequestInFlight) return weatherRequestInFlight;

  weatherRequestInFlight = fetch(weatherUrl)
    .then((response) => {
    ...
    .then((payload) => {
      ...
      weatherMemoryCache = weather;
      AsyncStorage.setItem(
        WEATHER_CACHE_KEY,
        JSON.stringify({ weather, cachedAt: Date.now() })
      ).catch(() => {});
      return weather;
    })
    .finally(() => {
...
export default function JaenWeatherForecast({ variant = "panel", onWeatherChange }) {
  ...
  const [loading, setLoading] = useState(!weatherMemoryCache);
  ...
  const fetchWeather = useCallback(
    async (isRefresh = false) => {
      try {
        if (isRefresh) {
          setRefreshing(true);
        } else if (!weatherMemoryCache) {
          setLoading(true);
        }
        setError("");

        const nextWeather = await requestFreshWeather(weatherUrl);
        setWeather(nextWeather);
      } catch (err) {
        if (!weatherMemoryCache) {
