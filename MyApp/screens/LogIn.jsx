...
export default function LogIn({ navigation }) {
  ...
  const getLoginErrorMessage = (err) => {
    ...
    if (text.includes("timeout") || err?.code === "ECONNABORTED") {
      return "The server is taking longer than expected to start. Please try again.";
    }
  ...
  const handleLogin = async () => {
    ...
    if (isSubmitting || loginInFlightRef.current) return;
    ...
    try {
      ...
      const res = await api.post(
        ...
        {
          // Render's free service can need more than the global 30-second
          // timeout for its first request after sleeping.
          timeout: 75000,
        }
      );
      const data = res.data || {};
      if (__DEV__) {
        console.log(`[performance] login request completed in ${Date.now() - loginStartedAt}ms`);
      }
